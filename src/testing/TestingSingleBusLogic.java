...
public class TestingSingleBusLogic {
	...
	public void gameTestNoRot() {
		...
		Game g = new Game(3);
		Circuit not = Circuit.ALL_CIRCUITS.get("Not").clone();
		Circuit or = Circuit.ALL_CIRCUITS.get("Or").clone();
		g.add(or, 1, 0);
		...
		// +-+-+-+-+-
		g.clear();
		Circuit not2 = Circuit.ALL_CIRCUITS.get("Not").clone();
		g.add(not2, 1, 0);
	...
	public void gameTestRot() {
		...
		Circuit or = Circuit.ALL_CIRCUITS.get("Or").clone();
		or.addRot(1);
		Circuit out = Circuit.ALL_CIRCUITS.get("Output").clone();
		out.addRot(1);
		g.add(or, 1, 1);
		assertFalse(or.outputList().get(0));
		g.add(not, 1, 0);
		...
		g.clear();
		not.setRot(1);
		not2 = Circuit.ALL_CIRCUITS.get("Not").clone();
		not2.setRot(0);
		Circuit and = Circuit.ALL_CIRCUITS.get("And").clone();
		g.add(and, 1, 1);
	...
	public void singleCircuitTest() {
		Circuit c = Circuit.ALL_CIRCUITS.get("Not");
		c.eval();
	...
	public void printCircuitTest() {
		Circuit or = Circuit.ALL_CIRCUITS.get("Or");
		String[] str = or.printCircuit();
		assertEquals(str[0], " I ");
		assertEquals(str[1], "IoO");
		assertEquals(str[2], "   ");

		Circuit not = Circuit.ALL_CIRCUITS.get("Not");
		not.addRot(3);
	...
	public void moveTest() {
		Game g = new Game(3);
		g.add(Circuit.ALL_CIRCUITS.get("Input").clone(), 0, 1);
		g.add(Circuit.ALL_CIRCUITS.get("Bus").clone(), 1, 1);
		g.add(Circuit.ALL_CIRCUITS.get("Output").clone(), 2, 1);
		g.toggle(0, 1);
		assertTrue(g.outputBusAtDir(1).get(0));
