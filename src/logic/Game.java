...
public class Game extends Observable{
	...
	public Game(String datum) {
		...
		for (int i = 2; i < data.length; i++) {
			...
			Circuit c = Circuit.ALL_CIRCUITS.get(cData[0]).clone();
	...
	public void rotate(int i, int j, int rot) {
		/*
		 * adds rot to circuit in game ORiginal code tileGrid[i][j].addRot(rot);
		 * updateGame(i, j); I shouldn't have to go to all this trouble to
		 * rotate a tile. Why aren't the children of the circuit picking up on
		 * the rotational change?
		 */
	...
	public List<Circuit> getAdj(int i, int j) {
		// gets all adjacent circuits. If circuit at dir not valid, circuit is
	...
	public void updateGame(int i, int j) {
		...
		if (c != null) {
			c.updateInputs();
			// then update the circuits around it that c outputs to (to prevent
			// infinite loops)
	...
	public Bus outputBusAtDir(int dir) {
		...
		outputs = outputs.stream().filter(c -> c.name.equals("Output"))
				.collect(toList());
		Bus tbr = new Bus();
		for (Circuit c : outputs) {
