...
public class Circuit {
	public static final Circuit Input = new Input();
	public static Map<String, Circuit> ALL_CIRCUITS;
	...
	public void updateInputs() {
		// if there is no game enclosing the circuit, updating inputs is
		// undefined
		if (game == null)
			return;
		...
		for (int dir = 0; dir < 4; dir++) {
			...
			if (b != null)
				inputBus.set(mod4(dir - rot), b.reverse());
			else
				inputBus.set(mod4(dir - rot),
						new Bus(inputBus.get(mod4(dir - rot)).size()));
		}
		eval();
	}

	public Bus validInputAtDir(int dir) {
		// If there is a valid output from another circuit at dir return it.
		// Else return null.
	...
	public List<Boolean> inputList() {
		return flatten(inputBus);
	}

	public List<Boolean> outputList() {
		return flatten(outputBus);
	}
	...
	public String outputAsString(int direction) {
		// given a (relative) direction, return the eval string for the inputs
		// at that direction
		String tbr = "";
		...
		for (int dir = 0; dir < 4; dir++) {
			...
			if (parent == null || this.validInputAtDir(mod4(dir + rot)) == null) {
				...
				for (int i = 0; i < inputBus.get(dir).size(); i++) {
					String s = parent.outputAsString(mod4(dir - rot - 2
							+ parent.rot));
					tbr += s + " ";
				}
		}
		...
		default:
			if(this.outputList().size() <= 1)
				...
				for(int i = 0; i < direction; i++)
					if(this.outputBus.size() > 0)
						outCount++;
				return tbr + name + ">"+ outCount;
			}
		}
	}
	...
	public boolean equiv(Circuit c) {
		for (int i = 0; i < 4; i++) {
			if (inputBus.get(i).size() != c.inputBus.get(i).size())
				return false;
			if (outputBus.get(i).size() != c.outputBus.get(i).size())
				return false;
		}
	...
	public String[] printCircuit() {
		...
		if (inputBus.get(mod4(0 - rot)).size() > 0)
			tbr[0] = " I ";
		else if (outputBus.get(mod4(0 - rot)).size() > 0)
			tbr[0] = " O ";
		if (inputBus.get(mod4(1 - rot)).size() > 0)
			tbr[1] = tbr[1].substring(0, 2) + "I";
		else if (outputBus.get(mod4(1 - rot)).size() > 0)
			tbr[1] = tbr[1].substring(0, 2) + "O";
		if (inputBus.get(mod4(2 - rot)).size() > 0)
			tbr[2] = " I ";
		else if (outputBus.get(mod4(2 - rot)).size() > 0)
			tbr[2] = " O ";
		if (inputBus.get(mod4(3 - rot)).size() > 0)
			tbr[1] = "I" + tbr[1].substring(1, 3);
		else if (outputBus.get(mod4(3 - rot)).size() > 0)
			tbr[1] = "O" + tbr[1].substring(1, 3);
