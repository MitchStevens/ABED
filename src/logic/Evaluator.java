...
public class Evaluator {
	...
	public static List<Token> tokenize(String str) throws Error{
		...
		for(String s : tokStr){
			switch(s){
			case "&": tokens.add(Operation.and);  continue;
			case "|": tokens.add(Operation.or);   continue;
			case "~": tokens.add(Operation.not);  continue;
			case "t":
			case "T": tokens.add(new Flag(true)); continue;
			case "f":
			case "F": tokens.add(new Flag(false)); continue;
			}
	...
	public boolean equiv(Evaluator e){
		//checks if the evals are logically identical using a probabilistic algorithm.
		//would like to get a exact answer but function doesn't scale well, would have to check
